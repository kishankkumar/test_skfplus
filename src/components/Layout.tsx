...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navigationItems = [
    { path: isLoggedIn ? '/dashboard' : '/', label: 'Home' },
    { path: '/meal-builder', label: 'Order' },
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-40 border-b border-primary-200/60 bg-white/95 backdrop-blur-lg">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white border-t border-primary-200/60 animate-slide-down">
            <div className="px-4 py-4 space-y-1">
              {navigationItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center px-4 py-3 rounded-lg text-sm font-medium transition-all duration-250 ${
                    isActivePath(item.path)
                      ? 'bg-primary-900 text-white'
                      : 'text-primary-700 hover:text-primary-900 hover:bg-primary-100'
                  }`}
              ...
              <div className="border-t border-primary-200 pt-4 mt-4">
                {isLoggedIn ? (
                  <div className="space-y-2">
                    <div className="flex items-center space-x-3 px-4 py-2.5 bg-primary-100 rounded-lg">
